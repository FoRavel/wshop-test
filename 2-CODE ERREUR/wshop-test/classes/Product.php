...
class Product // Fix: Rename ProductAction to Product
{
    /**
     * The table name
     *
     ...
     * @var     string
     */
    protected static $pk_name = 'produit_id'; // Fix: Remove "s"
	...
	{
		$sql_get = "SELECT p.* FROM " . self::$table_name . " p LIMIT " . $begin. ", " . $end; // Fix: Add table alias in FROM clause
	...
	public function delete() 
	{
		...
		return $this->db->query($sql_delete, [$id]); // Fix: Put Id in array
	}
    ...
    private function _dispatch()
    {
        ...
        $sql_dispatch = "SELECT p.*, 
            ...
            FROM produit p, produit_lang pl
            WHERE p.produit_id = :produit_id
            AND pl.fk_lang_id = :lang_id
            AND pl.fk_produit_id = p.produit_id"; // Fix: Remove ";" and place AND clauses after WHERE clause

        $params = [
            'produit_id' => $array_datas['produit_id'],
            'lang_id' => 1, // Fix: Add lang_id params
        ];
